...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("openJpgToPdfBtn")?.addEventListener("click", () => {
    jpgToPdfSelectedFiles = null;
    const fileCount = document.getElementById("jpgToPdfFileCount");
    if (fileCount) fileCount.textContent = "";
  ...
  document.getElementById("closeJpgToPdfBtn")?.addEventListener("click", () => {
    const modal = document.getElementById("jpgToPdfModal");
    if (modal) modal.style.display = "none";
  });
  const jpgToPdfUploadCard = document.getElementById("jpgToPdfUploadCard");
  const jpgToPdfInput = document.getElementById("jpgToPdfInput");
  if (jpgToPdfUploadCard && jpgToPdfInput) {
    jpgToPdfUploadCard.addEventListener("click", () => jpgToPdfInput.click());
    jpgToPdfInput.addEventListener("change", (event) => {
      const files = Array.from(event.target.files || []).filter((file) => file.type.startsWith("image/"));
      if (!files.length) {
        showToast("Sirf image files chunein.", "error");
        jpgToPdfSelectedFiles = null;
        return;
      }
      jpgToPdfSelectedFiles = files;
      const fileCount = document.getElementById("jpgToPdfFileCount");
      if (fileCount) fileCount.textContent = `${files.length} image(s) selected`;
    });
  }
