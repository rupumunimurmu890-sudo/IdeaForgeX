// ============================================================
// IdeaForgeX - Main JavaScript v11.0
// Cleaned + Fixed + Analytics + Tools + Projects
// ============================================================
...
function renderHistory() {
  ...
  const section =
    document.getElementById("ideaHistorySection");

  const row =
    document.getElementById("ideaHistoryRow");
  ...
  if (!list.length) {
    row.innerHTML = `
      <div class="empty-state">
        No past ideas yet. Generate your first report!
      </div>
    `;
...
const UI_STRINGS = {
  en: {
    tagline: "One AI Workspace for Everything",
    askAiBtn: "➤ Ask AI"
  },

  hi: {
    tagline: "सबके लिए AI वर्कस्पेस",
...
function renderReport(report) {
  ...
  const overall =
    document.getElementById("scoreOverall");
  ...
  const container =
    document.getElementById("reportSections");
...
async function generateReport() {
  ...
  if (!hasUsageRemaining()) {
    showToast(
      "Free Plan limit khatam! Pro upgrade karein.",
      "error"
    );
    return;
  }
...
async function copyText(text) {
  if (!text) {
    showToast(
      "Copy karne ke liye text nahi hai.",
      "error"
    );
    return;
  }
...
function smartRouteInput(input) {
  ...
  // Translation
  if (
    /\b(translate|translation|anuvad|anuvad|meaning|in hindi|in english|hindi me|english me)\b/.test(text)
  ) {
    return "translate";
  }
  ...
  // Writing
  if (
    /\b(write|draft|email|letter|essay|application|likho|likhna|message)\b/.test(text)
  ) {
    return "writing";
  }
  ...
  // Education
  if (
    /\b(explain|define|what is|kya hai|history|science|study|padhai|question|answer)\b/.test(text)
  ) {
    return "student";
  }
  ...
  // Code
  if (
    /\b(code|coding|program|script|function|python|javascript|html|css|react|api|worker|cloudflare|github)\b/.test(text)
  ) {
    return "code";
  }
  ...
  // Logo
  if (
    /\b(logo|brand logo|icon|symbol|emblem)\b/.test(text)
  ) {
    return "logo";
  }
  ...
  // Social
  if (
    /\b(post|tweet|instagram|linkedin|facebook|social media|caption|reel)\b/.test(text)
  ) {
    return "social";
  }
  ...
  // Image
  if (
    /\b(image|picture|photo|draw|generate image|banaiye|tasveer|chitra|painting)\b/.test(text)
  ) {
    return "ai-image";
  }
  ...
  // Marketing
  if (
    /\b(promote|marketing|advertisement|advertising|ad campaign|campaign|business promote)\b/.test(text)
  ) {
    return "autopilot";
  }
  ...
  // Document
  if (
    /\b(document|pdf|analyze document|summary|notes|report)\b/.test(text)
  ) {
    return "document";
  }
  ...
  // Goal
  if (
    /\b(goal|plan|roadmap|kaise karein|how to achieve|target|strategy)\b/.test(text)
  ) {
    return "goalplan";
  }
  ...
  // Money
  if (
    /\b(profit|loss|money|investment|roi|kamaai|income|expense|business calculation)\b/.test(text)
  ) {
    return "moneycalc";
  }
  ...
  // Improve
  if (
    /\b(improve|feedback|suggestion|better idea|idea improve)\b/.test(text)
  ) {
    return "improveidea";
  }
  ...
  // Roast
  if (
    /\b(roast|shark tank|critique|brutal|idea check)\b/.test(text)
  ) {
    return "roast";
  }
  ...
  // Poster
  if (
    /\b(poster|banner|flyer|graphic design)\b/.test(text)
  ) {
    return "poster";
  }
  ...
  // Video
  if (
    /\b(video|video script|youtube|shorts|tiktok)\b/.test(text)
  ) {
    return "video";
  }
  ...
  // Workflow
  if (
    /\b(workflow|batch|all in one|complete pack)\b/.test(text)
  ) {
    return "workflow";
  }
  ...
  // Cold email
  if (
    /\b(cold email|outreach|follow up|proposal|networking email)\b/.test(text)
  ) {
    return "email";
  }
  ...
  // Quote Card
  if (
    /\b(card|quote card|quote|instagram story|status|shareable)\b/.test(text)
  ) {
    return "card";
  }
  ...
  // Business Agent
  if (
    /\b(agent|startup|business idea|business start|shuru karna|start business)\b/.test(text)
  ) {
    return "agent";
  }
  ...
  // Chat
  if (
    /\b(chat|talk|conversation|baat karo|help me)\b/.test(text)
  ) {
    return "chat";
  }
...
function openToolWorkspace(tool) {
  ...
  if (imageWorkspace) {
    imageWorkspace.style.display =
      "none";
  }

  if (documentWorkspace) {
    documentWorkspace.style.display =
      "none";
  }
  ...
  if (tool === "chat") {
    ...
    if (
      bilingual &&
      bilingual.parentElement
    ) {
      bilingual.parentElement.style.display =
        "none";
    }
...
function renderUsageBanner() {
  ...
  if (remaining <= 0) {
    text.textContent =
      "⚠️ Free Plan Limit Reached! Upgrade to Pro for unlimited access.";
    ...
    text.textContent =
      `Free Plan • ${remaining} AI uses remaining today`;
...
function formatToolResult(
  ...
  if (tool === "code") {
    // Safely display code
    return `
      <pre class="code-result"><code>${escapeHtml(
        value
      )}</code></pre>
    `;
  }
...
function renderGoalResult(plan) {
  ...
  const sections = [
    ...
    },
    {
      ...
      title: "🛠️ Resources Needed"
    },
    {
      key: "POTENTIAL_OBSTACLES",
      title: "⚠️ Potential Obstacles"
    }
  ];
...
function renderMoneyResult(calc) {
  ...
  const sections = [
    ...
    {
      key: "PROFIT_PROJECTION",
      title: "📈 Profit Projection"
    },
    {
      key: "BREAK_EVEN",
      title: "⚖️ Break-Even Point"
    },
    {
      key: "RISKS",
      title: "⚠️ Financial Risks"
    }
  ];
...
async function sendChatMessage() {
  ...
  if (!hasUsageRemaining()) {
    showToast(
      "Free Plan limit khatam! Pro lein.",
      "error"
    );
    return;
  }
...
async function runBusinessAgent() {
  ...
  if (!currentProjectId) {
    showToast(
      "Pehle ek Project create karein!",
      "error"
    );
    return;
  }
  ...
  try {
    ...
    showToast(
      "🎉 Full Business Pack Generated & Saved!",
      "success"
    );
...
async function runAiTool(
  ...
  // IMPORTANT FIX:
  // Regenerate ke liye complete payload save
  lastToolPayload = {
    ...payload,
    tool,
    input: input.trim()
  ...
  try {
    ...
    if (tool === "autopilot") {
      ...
      if (
        ...
        throw new Error(
          data.error ||
          "Autopilot package generate nahi hua."
        );
      }
    ...
    if (
      ...
      showToast(
        `✨ ${TOOL_TITLES[effectiveTool] || "Tool"} detected`,
        "info"
      );
    }
    ...
    // ========================================================
    // OPTIONAL STRUCTURED RESULT RENDERING
    // ========================================================
...
async function remixContent(style) {
  if (
    ...
    showToast(
      "Text content par hi Remix kaam karta hai.",
      "error"
    );
    return;
  }
...
async function makeItBetter() {
  if (
    ...
    showToast(
      "Text content par hi Make Better kaam karta hai.",
      "error"
    );
    return;
  }
      ...
      await fetch(
        "/api/remix",
        {
          ...
          body: JSON.stringify({
            ...
            style:
              "professional, detailed, clear, polished and highly improved",
...
async function analyzeDocument() {
  ...
  if (!text) {
    showToast(
      "Document upload karein ya text paste karein.",
      "error"
    );
    return;
  }
...
function speakResult() {
  if (
    ...
    showToast(
      "Text content ko hi bola ja sakta hai.",
      "error"
    );
    return;
  }
...
function deleteProject(id) {
  if (
    !confirm(
      "Kya aap sure hain ki is project ko delete karna hai?"
    )
  ) {
    return;
  }
...
function renderProjectDetail(id) {
  ...
  if (!assets.length) {
    container.innerHTML = `
      <div class="empty-state">
        No assets yet.
        Use Business Agent to generate your first pack!
      </div>
    `;
...
document.addEventListener(
  ...
  () => {
    ...
    document
      ...
      ?.addEventListener(
        ...
        () => {
          ...
          // IMPORTANT FIX:
          // Previously payload.input was missing.
          if (!lastToolPayload) {
            showToast(
              "Regenerate ke liye previous request nahi mili.",
              "error"
            );
            return;
          }
          ...
          runAiTool(
            lastToolPayload.input,
            lastToolPayload.tool
          );
        }
      );
    ...
    if (
      ...
      docUploadCard.addEventListener(
        "click",
        () => {
          docFileInput.click();
        }
    ...
    if (
      ...
      imageUploadCard.addEventListener(
        "click",
        () => {
    ...
    console.log(
      "🚀 IdeaForgeX v11.0 initialized successfully."
    );
  }
);
